...
module "security_settings" {
  ...
  vpc_id               = module.networking_settings.vpc_id
...
module "application" {
  ...
  public_subnetC_id  = module.networking_settings.subnet_c_id
  name_load_balancer = var.name_load_balancer
  sg_lb_id           = [module.security_settings.sg_http_id]
  vpc_id             = module.networking_settings.vpc_id
